...
impl TerrainRgbCase3 {
    ...
    pub fn decode_elevations(&mut self) {
        ...
        for i in 0..(self.rgba.len() / 4) {
            unsafe {
                *self.elevations.get_unchecked_mut(i) = -10000.
                    + 6553.6 * *self.rgba.get_unchecked(4 * i) as f64
                    + 25.6 * *self.rgba.get_unchecked(4 * i + 1) as f64
                    + 0.1 * *self.rgba.get_unchecked(4 * i + 2) as f64;
            }
        }
    }
}
